...
/*
 * Datos geográficos reales de los 33 departamentos de Colombia
 * (fuente: @svg-maps/colombia, CC-BY-4.0). Cada path traza el contorno
 * real de un departamento — no son óvalos aproximados.
 * Paleta degradada: tonos claros (norte) a oscuros (sur/Amazonía),
              ...
              {CITIES.map((c, i) => {
                ...
                return (
                  <g key={c.city} onClick={() => toggleManual(c.city)} style={{ cursor: 'pointer' }}>
                    ...
                    <motion.circle cx={c.x} cy={c.y} r="22" fill="transparent" stroke="#1A1A1A" strokeWidth="1.1"
                      animate={{ r: [17, 62], opacity: [0.6, 0] }}
                      transition={{ repeat: Infinity, duration: 2.8, delay: i * 0.45, ease: 'easeOut' }} />
                    <motion.circle cx={c.x} cy={c.y} r="22" fill="transparent" stroke="#1A1A1A" strokeWidth="0.84"
                      animate={{ r: [17, 84], opacity: [0.35, 0] }}
                      transition={{ repeat: Infinity, duration: 2.8, delay: i * 0.45 + 0.5, ease: 'easeOut' }} />

                    {/* Dot principal negro */}
                    <motion.circle cx={c.x} cy={c.y}
                      r={c.count >= 9 ? 31 : c.count >= 2 ? 25 : 21}
                      fill={active ? '#E8118A' : '#1A1A1A'}
                      stroke="rgba(255,255,255,0.7)" strokeWidth="2.8"
                      initial={{ scale: 0 }} animate={{ scale: 1 }}
                      transition={{ delay: 0.3 + i * 0.15, duration: 0.5, ease: [0.34, 1.56, 0.64, 1] }} />

                    {/* Número — blanco */}
                    <motion.text x={c.x} y={c.y + 1.5} textAnchor="middle" dominantBaseline="middle"
                      fontSize={c.count >= 9 ? '22' : '21'} fontWeight="900"
